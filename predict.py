...
def predict_frame(model, frame: pd.DataFrame) -> pd.Series:
    features = engineer_features(clean_data(frame.drop(columns=["load_id"], errors="ignore")))
    return predict(model, features)


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate validation and December predictions.")
    ...
    parser.add_argument("--template", type=Path, default=Path("data/validation-predictions-template.csv"))
    parser.add_argument("--december", type=Path, default=Path("data/december-chart-inputs.csv"))
    ...
    predictions = pd.DataFrame({"load_id": validation["load_id"], "predicted_rate": predict_frame(model, validation)})
    template[["load_id"]].merge(predictions, on="load_id", how="left").to_csv(args.output, index=False)
