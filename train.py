...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train and time-validate the freight rate model."
    )
    ...
    print(
        f"Training rows: {train_mask.sum():,}; time holdout rows: {test_mask.sum():,}"
    )
    ...
    report_metrics(
        ...
        pd.Series(linear_predictions, index=target.loc[test_mask].index),
    )
    validation_model, best_parameters = tune_model(
        features.loc[train_mask], target.loc[train_mask]
    )
    print(f"Best Random Forest parameters: {best_parameters}")
    report_metrics(
        "Random Forest",
        target.loc[test_mask],
        predict(validation_model, features.loc[test_mask]),
    )
